...
type ProviderKind = 'builtin' | 'cli' | 'http';
...
export function SystemStatus({ providers, cache, rateLimits, totalTokens }: Props) {
  ...
  const kindCounts = useMemo(() => {
    return providers.reduce(
      ...
      { builtin: 0, cli: 0, http: 0 } as Record<ProviderKind, number>,
    );
  }, [providers]);
  ...
  const groupedByKind = useMemo(() => {
    const out: Record<ProviderKind, DashboardData['providers']> = { builtin: [], cli: [], http: [] };
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {(['builtin', 'cli', 'http'] as ProviderKind[]).map((k) => (
          <KindPill
            key={k}
            label={kindLabel(k)}
            count={kindCounts[k]}
