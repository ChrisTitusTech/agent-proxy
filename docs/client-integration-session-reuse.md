# Client Integration Guide - Session Reuse with agent-proxy

**For developers building chat apps, agents, or any client that calls agent-proxy and needs conversation continuity.**

This guide covers how to integrate with agent-proxy's Codex CLI session reuse feature. Same target: keep the conversation alive across HTTP calls without re-sending the entire message history.
...
>
> agent-proxy keys threads by `X-Agent-Proxy-Session-Id` header (falling back to API key id). Without a unique header per user/room:
>
...
5. Persist `(user_id, room_id, agent_proxy_session_id)` in your own DB.

agent-proxy will automatically route the second and subsequent calls to `codex exec resume <thread_id>`, so Codex keeps the context.
...
agent-proxy resolves the session in this order:

1. `X-Agent-Proxy-Session-Id` request header - **validated** `^[A-Za-z0-9._:-]{1,128}$`, max 128 chars
...
The header is not authentication. **A client that knows another user's session-id can join that conversation** (because Codex routes by `thread_id`, which agent-proxy keys by your session-id). Mitigations:
...
chat_rooms
  id                          PK
  user_id                     FK
  agent_proxy_session_id         text, UNIQUE per (user_id, project)
  title                       text
  agent_proxy_thread_id          text, nullable      -- optional cache from response header
  created_at                  timestamp
  last_message_at             timestamp
```

You **do not** need to track every message inside Codex - agent-proxy/Codex handles context. But you **should** still persist messages in your own DB for:

- UI rendering (chat history)
- Search / RAG
- GDPR / audit / deletion
- Reconstructing a session if `agent_proxy_session_id` ever rotates
...
# agent-proxy is OpenAI-compatible
client = OpenAI(
...
def send_in_room(room_session_id: str, message: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        extra_headers={
            "X-Agent-Proxy-Session-Id": room_session_id,
        },
    )
...
> **Note**: `X-Agent-Proxy-Thread-Id` is only exposed on non-stream responses (SSE headers must be flushed before the thread id is captured). For stream calls, the session id is sufficient - agent-proxy handles thread routing internally.
...
const client = new OpenAI({
  ...
  apiKey: process.env.AGENT_PROXY_API_KEY!,
});
...
async function sendInRoom(sessionId: string, content: string) {
  const response = await client.chat.completions.create(
    ...
    {
      headers: { 'X-Agent-Proxy-Session-Id': sessionId },
    },
  );
  return response.choices[0].message.content;
}
...
async function chat(sessionId: string, message: string): Promise<string> {
  const res = await fetch('http://localhost:8300/v1/chat/completions', {
    ...
    headers: {
      ...
      'Authorization': `Bearer ${process.env.AGENT_PROXY_API_KEY}`,
...
```bash
# Inspect the X-Agent-Proxy-Thread-Id response header (non-stream only)
...
app.post('/api/rooms', requireAuth, async (req, res) => {
  const room = await db.chatRooms.create({
    ...
    agentProxySessionId: `sess-${nanoid(24)}`,
  });
  res.json(room);
});
```
...
- If a user is silent for >TTL and then sends a message, agent-proxy treats it as a **new thread** - the context is lost.
...
| `thread/resume failed: no rollout found` | First call ran with `ephemeral: true` | Mapping must have `enable_session_reuse: true` (agent-proxy auto-forces `ephemeral: false`) |
| Two users see each other's context | Same session id for both | Make session ids unique per `(user x room)` |
...
| Sandbox flag not applied on second call | `-s` and similar flags are ignored by `exec resume` | Set sandbox in the first call (agent-proxy auto-filters them on resume) |
...
A: For Chat Completions you must always include at least the current user message. agent-proxy/Codex maintains the prior turns on their side, so you don't need to replay older messages - but the `messages` array must contain the *current* input.
...
A: Drop messages in your DB. agent-proxy's in-memory session manager will expire entries on its own (TTL). If you need hard server-side deletion now, remove the matching jsonl files under `~/.codex/sessions/...`.

**Q: Does this work with the Anthropic API endpoint (`/v1/messages`)?**
A: Yes - both `/v1/chat/completions` and `/v1/messages` honor `X-Agent-Proxy-Session-Id`.
